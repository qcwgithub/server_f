...
namespace Script
{
    public class User_ResetName : Handler<UserService, MsgResetName, ResResetName>
    {
        public override MsgType msgType => MsgType._User_ResetName;
        public User_ResetName(Server server, UserService service) : base(server, service)
        {
        }
        ...
        public override async Task<ECode> Handle(MessageContext context, MsgResetName msg, ResResetName res)
        {
            ...
            if (user == null)
            {
                (e, user) = await this.service.LoadUser(msg.userId, null);
                if (e != ECode.Success)
                {
                    return e;
                }
            ...
            e = await this.service.DestroyUser(user, UserDestroyUserReason.ResetName);
            return e;
        }
