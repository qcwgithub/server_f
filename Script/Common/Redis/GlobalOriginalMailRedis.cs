...
namespace Script
{
    public class GlobalOriginalMailRedis : GWaitInitDataRedis<GroupServer>
    {
        ...
        public class ReceivedPlayerIds
        {
            ...
            string Key(longid mailId) => MailKey.GlobalOriginalMailReceivedPlayerIds(mailId);
            ...
            public async Task AddMany(longid mailId, HashSet<longid> receivedPlayerIds)
            {
            ...
            public async Task<bool> AddWhenNotExists(longid mailId, longid playerId, bool addToPersist)
            {
            ...
            public async Task<bool> IsReceived(longid mailId, longid playerId)
            {
        ...
        public class ExcludePlayerIds
        {
            ...
            IDatabase GetDb() => this.parent.GetDb();
            string Key(longid mailId) => MailKey.GlobalOriginalMailExcludePlayerIds(mailId);
            ...
            public async Task AddMany(longid mailId, HashSet<longid> excludePlayerIds)
            {
                await this.GetDb().HashSetAsync(this.Key(mailId), excludePlayerIds.Select(_ => new HashEntry(_, 1)).ToArray());
            }

            public async Task<bool> Exist(longid mailId, longid playerId)
            {
                return await this.GetDb().HashExistsAsync(this.Key(mailId), playerId);
            }
        }
        ...
        public class PersistReceivedPlayerIds
        {
            ...
            IDatabase GetDb() => this.parent.GetDb();
            string Key(longid mailId) => MailKey.GlobalOriginalMailPersistReceivedPlayerIds(mailId);

            public async Task Push(longid mailId, longid receivedPlayerId)
            {
        ...
        public class PersistExcludePlayerIds
        {
            ...
            IDatabase GetDb() => this.parent.GetDb();
            string Key(longid mailId) => MailKey.GlobalOriginalMailPersistExcludePlayerIds(mailId);

            public async Task Push(longid mailId, longid excludePlayerId)
            {
        ...
        public class AllIds
        {
            ...
            public async Task Add(int serverId, longid mailId)
            {
                await this.GetDb().SetAddAsync(this.Key(serverId), mailId);
            }

            public async Task Remove(int serverId, longid mailId)
            {
                await this.GetDb().SetRemoveAsync(this.Key(serverId), mailId);
            }

            public async Task<HashSet<longid>> GetAll(int serverId)
            {
            ...
            public async Task<bool> Contains(int serverId, longid mailId)
            {
