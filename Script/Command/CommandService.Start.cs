...
namespace Script
{
    ...
    {
        public override async Task<ECode> Start()
        {
            ...
            ECode e = await this.InitServiceConfigsUntilSuccess();
            try
            {
            ...
            finally
            {
                ...
                {
                    await this.data.CloseAllConnections();
                }

            }
            if (e != ECode.MonitorRunLoop)
            {
                this.logger.Info("exit after 1 second...");
        ...
        public List<ServiceTypeAndId> GetArg_Targets()
        {
            ...
            if (s == "all")
            {
                targets = this.data.thisServerServiceConfigs
                    .Select(sc => sc.Tai())
        ...
        async Task<ECode> ServerAction(string action)
        {
            ...
            if (targets.Count == 0)
            {
                this.logger.Info("targets.Count == 0");
                return ECode.Success;
            }

            foreach (var tai in targets)
            {
                if (null == this.data.current_resGetServiceConfigs.FindServiceConfig(tai.serviceType, tai.serviceId))
                {
                    this.logger.ErrorFormat("serviceId({0}) location not exist in redis, will do nothing!", tai.serviceId);
                    return ECode.Success;
                }
            }
            ...
            if (action == "shutdown")
            {
                ...
                this.logger.InfoFormat("sort targets {0}", JsonUtils.stringify(targets.Select(x => x.ToString()).ToArray()));
            }
            ...
            foreach (var tai in targets)
            {
                this.logger.Info("perform action to " + tai.ToString() + "...");

                int serviceId = tai.serviceId;
                ServiceConfig sc = this.data.current_resGetServiceConfigs.FindServiceConfig(tai.serviceType, tai.serviceId);
                ServiceConnection connection = this.GetServiceConnectionOrConnect(sc.serviceType, sc.serviceId, sc.inIp, sc.inPort);
                ...
                switch (action)
                {
                    ...
                    case "shutdown":
                        {
                            e = await this.PerformShutdown(new MsgCommon().SetLong("serviceId", serviceId).SetLong("force", this.GetArg_Int("force", false)));
                        }
                        break;
                    ...
                    case "printPendingMsgList":
                        {
                            e = await this.PerformGetPendingMsgList(new MsgCommon().SetLong("serviceId", serviceId));
                        }
                        break;
                    ...
                    case "showScriptVersion":
                        {
                            e = await this.PerformShowScriptVersion(new MsgCommon().SetLong("serviceId", serviceId));
                        }
                        break;
                    ...
                    case "reloadScript":
                        {
                            ...
                            e = await this.PerformReloadScript(new MsgCommon().SetLong("serviceId", serviceId).SetString("zip", zip == null ? string.Empty : zip));
                        }
                        break;
                    ...
                    case "reloadConfigs":
                        {
                            ...
                            var r = await this.commandConnectToOtherService.Request(serviceId, MsgType._Service_ReloadConfigs, msgReload);
                            e = r.e;
                        }
                        break;
                    ...
                    case "getReloadConfigOptions":
                        {
                            ...
                            var r = await this.commandConnectToOtherService.Request(serviceId, MsgType._Service_GetReloadConfigOptions, msgGet);
                            e = r.e;
                            ...
                            if (e == ECode.Success)
                            {
                                ...
                                {
                                    this.logger.InfoFormat("{0}) {1}", i + 1, resGet.files[i]);
                                }
                            }
                        }
                        break;
                    ...
                    case "gc":
                        {
                            var msgGc = new MsgGC();
                            var r = await this.commandConnectToOtherService.Request(serviceId, MsgType._Service_GC, msgGc);
                            e = r.e;
                        }
                        break;
                    ...
                    case "userServiceAction":
                        {
                            ...
                            var r = await this.commandConnectToOtherService.Request(serviceId, MsgType._User_ServerAction, msg);
                            e = r.e;
                        }
                        break;
                    //
                    case "gatewayServiceAction":
                        {
                            ...
                            var r = await this.commandConnectToOtherService.Request(serviceId, MsgType._Gateway_ServerAction, msg);
                            e = r.e;
                        }
                        break;
                    ...
                    case "setGmFlag":
                        {
                            ...
                            e = await this.PerformSetPlayerGmFlag(new MsgCommon().SetLong("serviceId", serviceId).SetLong("startId", startId).SetLong("endId", endId));
                        }
                        break;
                    ...
                    case "showUserCount":
                        {
                            ...
                            var r = await this.commandConnectToOtherService.Request(serviceId, MsgType._User_GetUserCount, msg2);
                            e = r.e;
                            ...
                            if (e == ECode.Success)
                            {
                                ...
                                {
                                    this.logger.Info($"{kv.Key} = {kv.Value}");
                                }
                            }
                        }
                        break;
        ...
        async Task<ECode> UserAction(string action)
        {
            ...
            if (userId == 0)
            {
                this.logger.InfoFormat("userId is 0");
                return ECode.Success;
            }
            ...
            if (usId == 0)
            {
                this.logger.InfoFormat("playerId: {0} psId is null", userId);
                return ECode.Success;
            }

            this.logger.InfoFormat("playerId: {0} psId: {1}", userId, usId);

            ServiceConfig sc = this.data.current_resGetServiceConfigs.FindServiceConfig(ServiceType.User, usId);
            if (sc == null)
            {
                this.logger.ErrorFormat("serviceId({0}) location not exist in redis, will do nothing!", usId);
                return ECode.Success;
            }
            ...
            switch (action)
            {
                case "kick":
                    {
                        e = await this.PerformKick(new MsgCommon().SetLong("serviceId", usId).SetLong("playerId", userId));
                    }
                    break;
                case "saveUserInfoToFile":
                    {
                        e = await this.PerformSaveUserInfoToFile(new MsgCommon().SetLong("serviceId", usId).SetLong("playerId", userId));
                    }
                    break;
                ...
                case "un_fakeSilence":
                    {
                        ...
                        e = await this.PerformSetPlayerGmFlag(new MsgCommon().SetLong("serviceId", usId).SetLong("playerId", userId).SetString("msgGM", msgGMStr));
                    }
                    break;
