    ...
    public class Command_Start : OnStart<CommandService>
    {
        ...
        async Task<ECode> ServerAction(string action)
        {
            ...
            foreach (var tai in targets)
            {
                ...
                switch (action)
                {
                    ...
                    case "shutdown":
                        {
                            var r = await this.service.PerformShutdown(new MsgCommon().SetLong("serviceId", serviceId).SetLong("force", this.GetArg_Int("force", false)));
                            e = r.e;
                        }
                        break;
                    ...
                    case "printPendingMsgList":
                        {
                            var r = await this.service.PerformGetPendingMsgList(new MsgCommon().SetLong("serviceId", serviceId));
                            e = r.e;
                        }
                        break;
                    ...
                    case "showScriptVersion":
                        {
                            var r = await this.service.PerformShowScriptVersion(new MsgCommon().SetLong("serviceId", serviceId));
                            e = r.e;
                        }
                        break;
                    ...
                    case "reloadScript":
                        {
                            ...
                            var r = await this.service.PerformReloadScript(new MsgCommon().SetLong("serviceId", serviceId).SetString("zip", zip == null ? string.Empty : zip));
                            e = r.e;
                        }
                        break;
                    ...
                    case "setGmFlag":
                        {
                            ...
                            var r = await this.service.PerformSetPlayerGmFlag(new MsgCommon().SetLong("serviceId", serviceId).SetLong("startId", startId).SetLong("endId", endId));
                            e = r.e;
                        }
                        break;
        ...
        async Task<ECode> UserAction(string action)
        {
            ...
            switch (action)
            {
                case "kick":
                    {
                        var r = await this.service.PerformKick(new MsgCommon().SetLong("serviceId", usId).SetLong("playerId", userId));
                        e = r.e;
                    }
                    break;
                case "saveUserInfoToFile":
                    {
                        var r = await this.service.PerformSaveUserInfoToFile(new MsgCommon().SetLong("serviceId", usId).SetLong("playerId", userId));
                        e = r.e;
                    }
                    break;
                ...
                case "un_fakeSilence":
                    {
                        ...
                        var r = await this.service.PerformPlayerGM(new MsgCommon().SetLong("serviceId", usId).SetLong("playerId", userId).SetString("msgGM", msgGMStr));
                        e = r.e;
                    }
                    break;
