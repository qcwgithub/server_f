    ...
    public async Task<ECode> Save(long roomId, SceneRoomInfoNullable infoNullable)
    {
        ...
        if (infoNullable.messageId != null)
        {
            ...
            var upd = messageId_Db != null
                ? Builders<SceneRoomInfo_Db>.Update.Set(nameof(SceneRoomInfo_Db.messageId), messageId_Db)
                : Builders<SceneRoomInfo_Db>.Update.Unset(nameof(SceneRoomInfo_Db.messageId));
            updList.Add(upd);
        }
